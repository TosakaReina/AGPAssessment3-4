...
public:
	...
	TArray<FVector> GetPathToBestCover(const FVector& StartLocation, const FVector& TargetLocation);
...
private:
	...
	float MaxAcceptableAngle = 20.0f;
	float AngleWeight = 50.0f;
	...
	ANavigationNode* FindBestCoverNode(const FVector& TargetLocation, const FVector& PlayerLocation); 
	...
	static float CalculateJumpCost(const ANavigationNode* FromNode, const ANavigationNode* ToNode);
	float CalculateCoverScore(const FVector& TargetLocation, const FVector& PlayerLocation, ANavigationNode* CoverNode) const;
